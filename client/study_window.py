...
class StudyEditDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.study_date_input = QDateEdit()
        self.study_date_input.setCalendarPopup(True)
        self.study_date_input.setDisplayFormat("yyyy-MM-dd")
        self.study_date_input.setDate(QDate.currentDate())
        self.topic_input = QLineEdit()
        self.host_input = QLineEdit()
        self.speaker_input = QLineEdit()
        self.location_input = QLineEdit()
        self.study_method_input = QComboBox()
        for method in STUDY_METHODS:
            self.study_method_input.addItem(method or "未设置", method)
        self.participant_count_input = QSpinBox()
        self.participant_count_input.setRange(0, 999)
        self.participants_input = QLineEdit()
        self.content_input = QTextEdit()
        self.content_input.setPlaceholderText("直接记录本次学习的核心内容、重点知识、讨论要点和实践要求。")
        self.content_input.setMinimumHeight(250)
        self.notes_input = QTextEdit()
        self.notes_input.setPlaceholderText("补充备注、后续安排、资料来源或需跟进事项。")
        ...
        self.study_date_input.setMinimumHeight(40)
        self.study_date_input.setStyleSheet("font-size: 12pt; padding: 4px 8px;")
        self.study_method_input.setMinimumHeight(40)
        self.study_method_input.setStyleSheet("font-size: 12pt; padding: 4px 8px;")
        ...
        for text_edit in (self.content_input, self.notes_input):
        ...
        base_info_grid.addWidget(self.topic_input, 0, 0)
        base_info_grid.addWidget(self.study_method_input, 0, 1)
        base_info_grid.addWidget(self.host_input, 1, 0)
        ...
        meta_grid.addWidget(self.study_date_input, 0, 0)
        meta_grid.addWidget(self.participant_count_input, 0, 1)

        form.addRow("关联任务：", task_layout)
        form.addRow("记录编号：", self.record_no_input)
        form.addRow("时间与人数：", meta_grid)
        form.addRow("基础信息：", base_info_grid)
        form.addRow("学习内容：", self.content_input)
        form.addRow("备注说明：", self.notes_input)
    ...
    def open_for_edit(self, record):
        self.record = record
        self.setWindowTitle("编辑业务学习记录")
        self._reset_form()
        self.load_pending_tasks()
        self.record_no_input.setText(record.get("record_no") or "")
        self.study_date_input.setDate(self._to_qdate(record.get("study_date")))
        self.topic_input.setText(record.get("topic") or "")
        self.host_input.setText(record.get("host") or "")
        ...
        self.content_input.setPlainText(record.get("content") or "")
        self.notes_input.setPlainText(record.get("notes") or "")
        self._set_current_method(record.get("study_method") or "")
    ...
    def _reset_form(self):
        ...
        self._set_current_method("")

    def _set_current_method(self, method):
        index = self.study_method_input.findData(method)
        self.study_method_input.setCurrentIndex(index if index >= 0 else 0)
    ...
    def _select_task(self):
        ...
        for index, task in enumerate(self.pending_tasks, start=1):
            if task.get("id") == current_task_id:
                current_index = index
                break
        choice, ok = QInputDialog.getItem(self, "选择关联任务", "待完成的业务学习任务", options, current_index, False)
        if not ok:
            return
        if choice == options[0]:
            self.task_button.setProperty("task_id", None)
    ...
    def _submit(self):
        payload = {
            ...
            "study_date": self.study_date_input.date().toString("yyyy-MM-dd"),
            "topic": self.topic_input.text().strip(),
            "host": self.host_input.text().strip(),
            "speaker": self.speaker_input.text().strip(),
            "location": self.location_input.text().strip(),
            "study_method": self.study_method_input.currentData() or "",
            "participant_count": self.participant_count_input.value(),
            "participants": self.participants_input.text().strip(),
            "content": self.content_input.toPlainText().strip(),
            "notes": self.notes_input.toPlainText().strip(),
        }
        if not payload["study_date"] or not payload["topic"]:
            QMessageBox.warning(self, "提示", "请填写学习日期和主题")
            return
        try:
            if self.record:
                self.api.update_study(self.record["id"], payload)
            else:
                self.api.create_study(payload)
        except Exception as exc:
            self.app_context.handle_api_error(exc, self, "保存业务学习失败")
            return
...
class StudyDetailDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._init_ui()

    def _init_ui(self):
        self.setWindowTitle("业务学习详情")
        self.resize(800, 680)
        layout = QVBoxLayout(self)
    ...
    def open_record(self, record):
        summary_lines = [
            ...
            f"学习日期：{record.get('study_date') or '-'}",
            f"学习主题：{record.get('topic') or '-'}",
            f"学习方式：{record.get('study_method') or '-'}",
            f"主持人：{record.get('host') or '-'}",
            f"主讲人：{record.get('speaker') or '-'}",
            f"学习地点：{record.get('location') or '-'}",
...
class StudyWindow(QMainWindow):
    def __init__(self, app_context, parent=None):
        ...
        self.setWindowTitle("业务学习")
        self._restore_geometry()
        self._init_ui()

    def _restore_geometry(self):
        self.resize(int(self.config.get("study_window_width", 1220) or 1220), int(self.config.get("study_window_height", 760) or 760))
        x = self.config.get("study_window_x")
        y = self.config.get("study_window_y")
    ...
    def _init_ui(self):
        ...
        header = QHBoxLayout()
        self.summary_label = QLabel("业务学习加载中...")
        ...
        self.table.setHorizontalHeaderLabels(["编号", "日期", "主题", "方式", "主持人", "主讲人", "人数", "地点", "内容", "记录人"])
    ...
    def closeEvent(self, event):
        pos = self.pos()
        size = self.size()
        self.config["study_window_x"] = pos.x()
        self.config["study_window_y"] = pos.y()
        self.config["study_window_width"] = size.width()
        self.config["study_window_height"] = size.height()
    ...
    def delete_record(self):
        ...
        if not record:
            QMessageBox.information(self, "提示", "请先选择一条记录")
            return
        reply = QMessageBox.question(self, "确认删除", f"确认删除记录“{record.get('record_no') or record.get('topic') or record.get('id')}”吗？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply != QMessageBox.Yes:
            return
        try:
            self.api.delete_study(record["id"])
        except Exception as exc:
            self.app_context.handle_api_error(exc, self, "删除业务学习失败")
            return
        self.refresh_content(silent=True)
    ...
    def export_record(self):
        ...
        if not record:
            ...
            return
        try:
            exported = self.api.export_study_word(record["id"])
        except Exception as exc:
            self.app_context.handle_api_error(exc, self, "导出 Word 失败")
            return
        default_name = exported.get("filename") or f"{record.get('record_no') or '业务学习记录'}.docx"
