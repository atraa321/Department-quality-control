...
title KSQC Backend Server 2016 Startup

echo ================================================
echo   KSQC Backend Startup ^(Windows Server 2016^)
echo ================================================
echo.
...
if not defined SYSTEM_PYTHON (
    ...
    echo         Install Python 3.12.x and add it to PATH.
    goto :fail
)

set "PY_VER="
for /f "tokens=2 delims= " %%i in ('"%SYSTEM_PYTHON%" --version 2^>^&1') do set "PY_VER=%%i"
if "%PY_VER:~0,4%" NEQ "3.12" (
    echo [ERROR] Detected Python version: %PY_VER%
    echo         This backend bundle targets Python 3.12.x on Windows Server 2016.
    echo         Install Python 3.12.x and retry.
    goto :fail
)
...
if exist "%WHEELHOUSE_DIR%\pip-25.0.1-py3-none-any.whl" (
    "%PYTHON%" -m pip install --disable-pip-version-check --no-warn-script-location --no-index --find-links="%WHEELHOUSE_DIR%" --upgrade "pip==25.0.1" "setuptools==69.5.1" "wheel==0.42.0" >> "%LOG_FILE%" 2>&1
) else (
    "%PYTHON%" -m pip install --disable-pip-version-check --no-warn-script-location --upgrade "pip==25.0.1" "setuptools==69.5.1" "wheel==0.42.0" >> "%LOG_FILE%" 2>&1
)
...
if exist "%WHEELHOUSE_DIR%" (
    "%PYTHON%" -m pip install --disable-pip-version-check --no-warn-script-location --no-index --find-links="%WHEELHOUSE_DIR%" -r "%ROOT%\server\requirements.txt" >> "%LOG_FILE%" 2>&1
) else (
    "%PYTHON%" -m pip install --disable-pip-version-check --no-warn-script-location -r "%ROOT%\server\requirements.txt" >> "%LOG_FILE%" 2>&1
)
if errorlevel 1 (
